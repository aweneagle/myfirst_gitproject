...
/* 启动守护 routine, 监听网络socket, 读取数据并执行回调函数
 *
 */
func (c *connect) Run () error {
	...
	for {
		...
		if err := c.listen_and_read(); err != nil {
			/* 客户端断开连接 | 其他异常情况出现 */
			if err == io.EOF {
...
func (c *connect) Request(req []byte, response []byte) (int, error) {
	...
	if err := c.runner.Request(r); err != nil {
		return 0, err
	} else {
...
func (conn *connect) listen_and_read () error {
